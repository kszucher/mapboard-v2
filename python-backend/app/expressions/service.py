...
def validate_expressions_for_node_type(node_type: str, expressions: list[models.Expression] | list[ExpressionCreate]) -> None:
    if node_type == NodeType.START:
        ...
        if len(expressions) != 1:
            raise ValidationError(f"{node_type} nodes must have exactly 1 expression")
...
async def create_expression(uow: UnitOfWork, data: ExpressionCreate) -> models.Expression:
    ...
    uow.emit(
        ...
        payload={
            ...
            "nodeId": node.id,
            "expression": {"id": expr.id, "idx": expr.idx, "raw_string": expr.raw_string},
        }
    )
    return expr

async def update_expression(uow: UnitOfWork, expression_id: uuid.UUID, data: ExpressionUpdate) -> models.Expression | None:
