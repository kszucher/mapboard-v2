...
Graphboard is a logic-driven graph editor for building agentic workflows. Unlike typical canvas editors, Graphboard enforces **fully programmatic auto-layout** (via ELK) and binds connections to expressions.
...
* **Resizing & Debounce**: Typing inline changes node height. To prevent layout stuttering, expression edits are debounced (1000ms). Once resizing stops, updated dimensions trigger a fresh ELK layout.

### 3. Node Expression Ports & Order Rules
Nodes do not have generic ports; they contain **Expressions** with explicit `is_input` and `is_output` flags (which render left target handles and right source handles, respectively).
* **Sequential Ordering Constraint**: Expressions inside a node must follow a strict flow-logic order:
  `Inputs -> Both -> None -> Outputs`
  Violations will trigger validation errors.
* **Hard Links**: Edge connections store the specific `from_expression_id` and `to_expression_id`. If expressions are reordered, the connecting wires stay attached to their logic rows.
...
* **Custom Handles Lifecycle & `updateNodeInternals`**: When expressions change type (adding/removing input/output handles), React Flow's DOM-cached registry becomes stale. We must compute a stable `expressionsHash` in `FlowNode.tsx` and run a `useEffect` triggering `updateNodeInternals(id)` whenever the hash changes to force React Flow to re-query the handles.
